#
# from https://gist.github.com/samneggs
#
...
def init_title():
    for i in range(0,9):
        ...
        t.append(init_obj(let_deg[i],let_rad[i],len(let_deg[i]),1,1+i*20,50,1))
        t[i].ax=0
        t[i].ay=0
        draw_object(t[i])
    ...
    while djb.pressed(djb.btn_Start)==0:
        djb.scan_jst_btn() #scan buttons
        for i in t:
            draw_object(i)
        djb.display.show()
...
def init_ast():                      # old dot asteroids
    for i in range(0,6):             # 6 number of asteriod dots
        a.append(Point())
        a[i].x=randint(0,160)        # random location
        a[i].y=randint(0,128)
...
def draw_object(obj):
    ...
    for i in range(0,obj.pts-1):
        djb.display.line(obj.pt[i].x,obj.pt[i].y,obj.pt[i+1].x,obj.pt[i+1].y,djb.display.BLACK) # erase old obj
    ...
    if (obj.exp == 0 and obj.tumble==0) or (obj.size>0 and obj.tumble!=0):
        ...
        if ship.damage==False:
            ...
            c=djb.display.GREEN_H
        for i in range(0,obj.pts-1):
            djb.display.line(obj.pt[i].x,obj.pt[i].y,obj.pt[i+1].x,obj.pt[i+1].y,c) # draw new obj
    token = 1
...
def move_object(obj):
    ...
    obj.y=obj.y+obj.ay
    if obj.x > 160:                 # wrap around if out of bounds
        obj.x = 0
    if obj.x < 0:
        obj.x = 160
    if obj.y > 128:
...
def buttons():
    ...
    if djb.pressed(djb.btn_Right):            # rotate ship CW
        ship.deg+=5
        if ship.deg>359:
            ship.deg=0
    if djb.pressed(djb.btn_Left):             # rotate ship CCW
        ship.deg-=5
        if ship.deg<0:
            ship.deg=359
    if djb.pressed(djb.btn_A):
        thrust()
...
def move_miss():  # !!! OLD !!!
    ...
    while len(m)>0 and i< len(m):                                   # have #miss and index<#miss
        if m[i].x >-1 and m[i].x<161 and m[i].y>-1 and m[i].y<129:  # check miss inbounds
            djb.display.pixel(int(m[i].x),int(m[i].y),djb.display.black)            # erase old miss 
            m[i].x=m[i].x+m[i].ax                                   # calc new miss
            m[i].y=m[i].y+m[i].ay
            djb.display.pixel(int(m[i].x),int(m[i].y),djb.display.white)            # draw new miss
            ...
            while len(a)>0 and j< len(a) and len(m)>0 and i<len(m): # have ast, indx<ast, have miss, indx<miss
                if m[i].x < a[j].x+3 and m[i].x > a[j].x-3 and m[i].y < a[j].y+3 and m[i].y > a[j].y-3 and a[j].exp == 0: # check collision
                    djb.display.pixel(int(m[i].x),int(m[i].y),djb.display.black)
                    djb.display.pixel(int(a[j].x),int(a[j].y),djb.display.black)
                    explode_cleanup()
...
def move_miss_new():
    ...
    while len(m)>0 and i< len(m):                                   # have #miss and index<#miss
        if m[i].x >-1 and m[i].x<161 and m[i].y>-1 and m[i].y<129:  # check miss inbounds
            ...
            m[i].x=m[i].x+m[i].ax                                   # calc new miss
            m[i].y=m[i].y+m[i].ay
            djb.display.pixel(int(m[i].x),int(m[i].y),djb.display.WHITE_H)            # draw new miss
            j=0                                                     # j index of asteroid a
...
def move_ast():    # !!! OLD !!! move dot asteroids
    i=0
    while i<len(a):
        djb.display.pixel(int(a[i].x),int(a[i].y),djb.display.BLACK)
        a[i].x=a[i].x+a[i].ax
        a[i].y=a[i].y+a[i].ay
        ...
        if a[i].y>127:
            a[i].y=0        
        if a[i].exp == 0:
            djb.display.pixel(int(a[i].x),int(a[i].y),djb.display.WHITE_H)
        else:
            explode()
            if len(e) == 0:
                d=a.pop(i)
...
def explode():    
    ...
    while len(e)>0 and i< len(e):    
        if e[i].x >-1 and e[i].x<161 and e[i].y>-1 and e[i].y<129 and e[i].exp<100:
            djb.display.pixel(int(e[i].x),int(e[i].y),djb.display.BLACK)
            e[i].x=int(e[i].x+e[i].ax)
            e[i].y=int(e[i].y+e[i].ay)
            djb.display.pixel(int(e[i].x),int(e[i].y),djb.display.WHITE_H)
            e[i].exp+=1
        else:
            djb.display.pixel(int(e[i].x),int(e[i].y),djb.display.BLACK)
