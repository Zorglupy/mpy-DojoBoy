#
# snake-djb-240.py Game
# by Billy Cheung  2019 10 26
# Modified by Yoyo Zorglup for DojoBoy
#
...
def drawGameMenu():
    global SNAKE_SIZE
    clearScreen()
    djb.display.center_text_XY('SNAKE', -1, 20, COLOR_APPLE, 2)
    djb.display.rect(djb.display.width//2 - 50,10,100,32, COLOR_WALL)
    
    djb.display.rect(djb.display.width - djb.max_vol * 5 - 4, 0, djb.max_vol * 5 + 2, 6,COLOR_SCORE)
    djb.display.fill_rect(djb.display.width - djb.max_vol * 5 - 3,1, djb.vol[0] * 5, 4,COLOR_APPLE)

    djb.display.text("Button A : START  ",0,60,COLOR_SCORE)
    if game['demo'] :
        djb.display.text('Button Down : AI-PLAYER', 0,80, COLOR_WALL)
    else :
        djb.display.text('Button Down : 1-PLAYER', 0,80, COLOR_SCORE)
    djb.display.text("Button Up : SIZE {}".format(SNAKE_SIZE),0,100,COLOR_SCORE)
    djb.display.text("Button M + U/D : FRAME {}".format(djb.display.frame_rate),0,120,COLOR_SCORE)
    djb.display.text("Button V + U/D : VOLUME",0,140,COLOR_SCORE)
    djb.display.text("Button U+D : EXIT",0,160,COLOR_APPLE)
