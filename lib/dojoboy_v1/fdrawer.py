...
class FontLoader:
    def __init__( self, font_name ):
        ...
        fsize = [ items[3] for items in os.ilistdir('font') if items[0].upper() == fname.upper() ]
        if len(fsize) < 1:
            raise Exception( "missing %s file" % fname )
        f = open( 'font/%s.bin' % font_name, 'rb' )
        data = f.read( 4 )
...
class FontDrawer:
    def __init__(self, frame_buffer, font_name  ):
        ...
        self._fontscale = 1
        self._fontcolor = 0xFFFF #(1) # Font color
        self._bgcolor   = None #(0) # Background color
    ...
    def _fill_bicolor(self, data, x, y, width, height, scale=1 ):
        ...
        for col in data:
            ...
            for _y in range( height-1, -1, -1 ):
                c = self._fontcolor if ((col & (1<<_y)) > 0) else self._bgcolor
                if c != None:
